...
def get_outlier_detection_config():
    config = load_config()
    return config.get('outlier_detection', {})

def get_feature_binning_config():
    config = load_config()
    return config.get('feature_binning', {})

def get_feature_encoding_config():
    config = load_config()
    return config.get('feature_encoding', {})

def get_feature_scaling_config():
    config = load_config()
...
def get_handle_imbalance_config():
    config = load_config()
...
def create_default_config() ->None:
    ...
    if not os.path.exists(config_path):
        default_config = {'data': {'file_path':
            'data/raw/telco.customer-dataset.xls', 'target_column': 'Churn',
